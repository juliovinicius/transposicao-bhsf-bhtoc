...
#perdas
perda = {'retirobaixo': [],
'tresmarias': [0.7 for i in range(744)],
'queimado': [2.91 for i in range(744)],
'sobradinho': [0.2 for i in range(744)],
'itaparica': [0.52 for i in range(744)],
'pamx': [0.6 for i in range(744)],
'xingo': [0.84 for i in range(744)],
'serradamesa': [1.27 for i in range(744)],
'canabrava': [0.9 for i in range(744)],
'saosalvador': [0.64 for i in range(744)],
'peixeangelical': [0.47 for i in range(744)],
'lajeado': [0.22 for i in range(744)],
'estreito': [0.39 for i in range(744)],
'tucurui': [0.29 for i in range(744)]}
...
for uhe in cotageo:
	if uhe in ('pamx', 'xingo', 'canabrava', 'saosalvador', 'estreito', 'lajeado'):
		cotageo[uhe] = [limitecotas[uhe][0] for i in range(744)]
...
for bacia in pcs:
	pcs[bacia]['full'] = []
	i = 0
	for per in pcs[bacia]['periodosconv']:
		aux = []
		aux.append(per[0]), aux.append(per[len(per)-1]), aux.append(len(per)), aux.append(pcs[bacia]['menorvalor'][i])
		i += 1
		pcs[bacia]['full'].append(aux)

#organizando os períodos críticos e enviando para os csv's
nomes = ['Início','Fim','Duração','Menor valor']
pcsfullbhsf = pd.DataFrame(pcs['BHSF']['full'], columns=nomes)
pcsfullbhtoc = pd.DataFrame(pcs['BHTOC']['full'], columns=nomes)
pcsfullbhsf.to_csv('pcsfullbhsf.csv',index=False)
pcsfullbhtoc.to_csv('pcsfullbhtoc.csv',index=False)
...
for uhe in mediaanoqnat:
	mediaanoqnat[uhe] = sum(mediaanoqnat[uhe]) / 62
...
def plotarENA(graf, serie, titulo):
	#graf.plot(t,serie)
	graf.fill_between(t,0,serie)
	graf.xaxis.set_major_locator(plt.MaxNLocator(20))
	graf.axis["right"].set_visible(False)
	graf.axis["top"].set_visible(False)
	graf.set_ylabel('\n\n\nENA (MWmed)')
...
graf1 = fig1.add_subplot(axes_class=Axes)
plotarENA(graf1, ENAbacia['BHSF'], 'ENA para BHSF / 1931-1992')
plt.xlim("Jan/1931", "Dez/1992")
plt.ylim([0, 45000])
plt.tight_layout()
plt.savefig('ENABHSFpre1993.png')

fig2 = plt.figure(figsize=(16,5))
graf2 = fig2.add_subplot(axes_class=Axes)
plotarENA(graf2, ENAbacia['BHTOC'], 'ENA para BHTOC / 1931-1992')
plt.xlim("Jan/1931", "Dez/1992")
plt.ylim([0, 45000])
plt.tight_layout()
plt.savefig('ENABHTOCpre1993.png')

fig3 = plt.figure(figsize=(16,5))
graf3 = fig3.add_subplot(axes_class=Axes)
plotarEAR(graf3, propEAR['BHSF'], '%EAR para BHSF / 1931-1992')
plt.xlim("Jan/1931", "Dez/1992")
plt.ylim(0, 105)
plt.tight_layout()
plt.savefig('EARBHSFpre1993.png')

fig4 = plt.figure(figsize=(16,5))
graf4 = fig4.add_subplot(axes_class=Axes)
plotarEAR(graf4, propEAR['BHTOC'], '%EAR para BHTOC / 1931-1992')
plt.xlim("Jan/1931", "Dez/1992")
plt.ylim(0, 105)
plt.tight_layout()
plt.savefig('EARBHTOCpre1993.png')
